...
def main() -> int:
    ...
    # 前置产物提示：如果用户直接跑截图流程但没有转录，提醒先运行 video-summary
    project_dir = find_project_dir(args.project_dir, video_path)
    runs_dir = project_dir / "runs"
    possible_runs = [p for p in runs_dir.iterdir() if p.is_dir()] if runs_dir.exists() else []
    has_transcript = any((p / "transcript.json").exists() for p in possible_runs)
    if not has_transcript and args.asr == "mock":
        print("⚠️  当前使用 mock ASR，转录不是真实的。", file=sys.stderr)
        print("   如需真实转录，请先运行 video-summary 或去掉 --asr mock。", file=sys.stderr)
