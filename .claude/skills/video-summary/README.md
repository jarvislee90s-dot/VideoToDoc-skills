...
B站视频下载通常需要代理才能成功，请在命令中添加 `--proxy` 参数：
...
**特别说明**：video-to-slides 需要视频文件作为输入。video-summary 下载的视频文件名是 `<视频标题>.mp4`（不是固定的 `video.mp4`）。如果 video-summary 通过字幕直接输出了 transcript（没有下载视频），你需要：
1. 确保原视频文件可访问
