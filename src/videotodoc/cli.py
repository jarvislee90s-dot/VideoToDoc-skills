...
from .feishu import publish_to_feishu
from .io import read_json
from .mindmap import render_mindmap_and_refresh_docs
from .pipeline import process_video, sections_from_dict
...
def build_parser() -> argparse.ArgumentParser:
    ...
    publish = subparsers.add_parser("publish", help="发布已有 run 目录到飞书")
    publish.add_argument("run_dir", type=Path)
    publish.add_argument("--config", type=Path, default=Path("config.yaml"))
    publish.add_argument("--target", choices=["feishu"], default="feishu")
...
def _publish(args: argparse.Namespace) -> int:
    ...
    markdown_path = _find_publish_markdown(run_dir)
    sections_path = _find_sections_path(run_dir)
    if not markdown_path.exists():
        raise VideoToDocError(f"找不到本地 Markdown：{markdown_path}")
    if not sections_path:
        raise VideoToDocError(f"找不到 sections 缓存：{run_dir / 'cache'}")
    sections = sections_from_dict(read_json(sections_path))
    title = args.title or run_dir.name
    doc_ref = publish_to_feishu(markdown_path, title, sections, settings, dry_run=args.dry_run)
    print("飞书发布完成：" if not args.dry_run else "飞书发布 dry-run：")
    print(doc_ref)
    return 0
...
def _find_sections_path(run_dir: Path) -> Path | None:
    cache_dir = run_dir / "cache"
    if not cache_dir.exists():
        return None
    matches = list(cache_dir.glob("*.sections.json"))
    if not matches:
        return None
    return max(matches, key=lambda path: (path.stat().st_mtime_ns, path.name))


def _find_publish_markdown(run_dir: Path) -> Path:
    for name in ("draft_semantic.md", "draft_compact.md", "draft.md"):
        path = run_dir / name
        if path.exists():
            return path
    return run_dir / "draft.md"
